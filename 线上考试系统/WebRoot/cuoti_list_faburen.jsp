...
// 以下是检测搜索框中是否填写了或者选择了什么，则写入sql 语句中
    ...
    where += " AND tikuid='"+request.getParameter("tikuid")+"' ";
                 ...
                 if(request.getParameter("leixing") != null && !"".equals( request.getParameter("leixing") )){ where+=" AND leixing LIKE '%"+request.getParameter("leixing")+"%'"; } 
                    List<HashMap> list = Query.make("cuoti").where(where).order(orderby+" "+sort).page(15);
...
<div class="panel panel-default">
    ...
    类型
                            ...
                            <input type="text" class="form-control" style="" name="leixing" value="<%= request.getParameter("leixing") !=null ? request.getParameter("leixing") : "" %>"/>                        
</div>
...
<td> <%= map.get("kaoshiren") %> </td>
                        ...
                        <td align="center">
                                                                                        ...
                                                                                        <a href="cuoti_detail.jsp?id=<%= map.get("id") %>">
                                    详细
                                </a>
                                                        <a href="cuoti_updt.jsp?id=<%= map.get("id") %>">修改</a>
                            <a href="cuoti.jsp?a=delete&id=<%= map.get("id") %>" onclick="return confirm('真的要删除？')">删除</a>
                            <!--qiatnalijne-->
                        </td>
                    </tr>
                    <% } %>
